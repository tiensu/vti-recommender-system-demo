...
def main():
    ...
    models = ['Content Base: Similar items based on image embeddings', 
              ...
              'Content Base: Similar items based discriptive features', 
              'Collaborative Filter: Similar items based on user-item utility matrix',
              'Hybrid Model: Similar items based on a combination of all embeddings']
    
    model_descs = ['Image embeddings are calculated using VGG16 CNN with Keras/Tensorflow', 
                  'Text description embeddings are calculated using "universal-sentence-encoder" with TensorFlow',
                  'Features embeddings are calculated by one-hot encoding the descriptive features provided by retail company',
                  'Tensorflow Recomender model performes a collaborative filtering based ranking using a neural network', 
